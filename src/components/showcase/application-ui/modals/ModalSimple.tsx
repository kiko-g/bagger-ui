...
export function ModalSimple({ startOpen }: Props) {
  ...
  return (
    <>
      <Button
        onClick={openModal}
        className="h:bg-black/70 flex items-center gap-1 rounded-md bg-black/40 px-3 py-2 text-sm font-medium text-white transition focus:outline-hidden data-focus:outline-1 dark:bg-white/30 dark:data-hover:bg-white/50"
      >
      ...
      <Dialog open={isOpen} as="div" className="relative z-100000 focus:outline-hidden" onClose={closeModal}>
        <div className="fixed inset-0 w-screen overflow-y-auto bg-black/50">
          <div className="flex min-h-full items-center justify-center p-4">
            <DialogPanel
              transition
              className="w-full max-w-lg rounded-xl bg-white p-6 backdrop-blur-2xl duration-300 ease-out data-closed:transform-[scale(95%)] data-closed:opacity-0 dark:bg-white/10"
            >
              ...
              <div className="text-sm/6 font-normal">
                ...
                <ul className="mt-4 ml-3 flex list-disc flex-col gap-y-0.5 lg:ml-4">
              ...
              <div className="mt-8 flex justify-end">
                <Button
                  className="inline-flex items-center gap-2 rounded-md bg-zinc-700 px-3 py-1.5 text-sm/6 font-semibold text-white shadow-inner shadow-white/10 focus:outline-hidden data-focus:outline-1 data-focus:outline-white data-hover:bg-zinc-600 data-open:bg-zinc-700"
                  onClick={closeModal}
                >
                  Got it, thanks!
                </Button>
